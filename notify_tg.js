...
const CHAT_ID = '-1001659279768' // Mods
...
if (e === 'success') {
  if (fs.existsSync('/tmp/apidata_iumessage')) {
    ...
    var url = encodeURI('https://api.telegram.org/bot' + BOT_TOKEN + '/editMessageText?message_id=929&chat_id=@moicovid&parse_mode=Markdown&text=' +
      apidataiutable)
    const settings = { method: 'Get' }
    fetch(url, settings).then(res => res.json())
      .then(json => console.log(json))
    url = encodeURI('https://api.telegram.org/bot' + BOT_TOKEN + '/sendMessage?chat_id=-1001659279768&parse_mode=Markdown&text=' +
      apidataiutable)
    fetch(url, settings).then(res => res.json())
      .then(json => console.log(json))
    url = encodeURI('https://api.telegram.org/bot' + BOT_TOKEN + '/sendmessage?' +
      'disable_web_page_preview=true&disable_notification=true&parse_mode=Markdown&chat_id=-1001659279768&text=' + apiDataIUmessage)
